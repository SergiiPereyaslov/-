...
Ці команди підтверджують, що запрацювало саме те, що має. Кожна з них
перевіряє конкретну знахідку аудиту — якщо якась дасть інший результат,
щось у конфігурації пішло не так.
...
curl -sI https://smartecopack.com/ | grep -iE \
  "content-security-policy|x-content-type-options|referrer-policy|strict-transport"
# очікується 4 рядки; X-Powered-By не має бути взагалі

# Неіснуючий слаг має давати 404, а не 500. П'ять адрес — п'ять «404».
for u in /upakovka/nemaye-takoho/ /dlya/nemaye-takoho/ /catalog/nemaye/ \
         /product/nemaye/ /blog/nemaye/; do
  curl -s -o /dev/null -w "$u -> %{http_code}\n" "https://smartecopack.com$u"
done

# Адмінка не кешується й не індексується
curl -sI https://smartecopack.com/admin/ | grep -iE "cache-control|x-robots-tag"

# Обмеження частоти: 12 запитів поспіль до приймання заявок.
# Перші кілька — 422 (payload навмисно невалідний, нічого не збережеться),
# далі мають піти 429. Якщо 429 не з'явився — limit_req не підхопився.
for i in $(seq 12); do
  curl -s -o /dev/null -w "%{http_code} " -X POST \
    https://smartecopack.com/api/lead/ \
    -H 'Content-Type: application/json' -d '{"phone":"невалідний"}'
done; echo

# Канал для дослідників безпеки
curl -s https://smartecopack.com/.well-known/security.txt
```

Ще одне, що робиться лише руками: переконайтесь, що застосунок **не**
слухає зовнішній інтерфейс. Юніт задає `HOSTNAME=127.0.0.1`, і саме це
не дає обійти nginx разом із його лімітами частоти.

```bash
ss -ltnp | grep 3000     # має бути 127.0.0.1:3000, а не 0.0.0.0:3000
```
...
Окремо перевірено обмеження частоти з `nginx.conf`: nginx був піднятий у
середовищі розробки, і серія запитів до `/api/lead/` дала
`200 200 200 200 200 200 429 429 429 429 429 429` — тобто `limit_req`
справді відсікає надлишок, а не просто описаний у конфізі.

Виправлення за другим аудитом безпеки перевірені на зібраному сайті:
неіснуючі слаги міст і сегментів тепер дають 404 замість 500 (по два
маршрути в кожному мовному дереві), розмітка JSON-LD екранована й лишається
валідним JSON (4 блоки на сторінці категорії), заголовки безпеки
віддаються самим застосунком, `/.well-known/security.txt` відповідає 200.
Браузерна перевірка показала 0 порушень CSP і робочі інлайн-скрипти теми
й A/B-тесту.

Не перевірено на реальному сервері: systemd-юніт, випуск сертифіката
Let's Encrypt і збірка Docker-образу (у середовищі розробки не було
запущеного демона Docker; крім того, Docker-шлях зараз зламаний — див.
[README.md](README.md)). Це стандартні конфігурації, але перший запуск
краще робити на тестовому сервері, а не одразу на бойовому.

Що лишилося з аудиту й свідомо не виправлено:

- `npm audit` показує 4 вразливості рівня high у піддереві CLI Prisma
  (`mysql2`, `deepmerge-ts`). У зібраний застосунок вони не потрапляють —
  `.next/standalone/node_modules` містить 26 пакетів, і цих серед них
  немає, а проєкт працює з PostgreSQL, не з MySQL. Зникне з оновленням
  Prisma.
- `/api/ab` приймає лічильники A/B-тесту без автентифікації. Персональних
  даних там не зберігається, зловживання обмежене зоною `sep_api`
  (120 запитів/хв), але накрутити статистику теоретично можна. Якщо
  рішення за тестом буде важливим — звіряйте з журналами сервера.
