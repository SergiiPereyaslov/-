...
/**
 * Перша лінія захисту розділу: сесія перевіряється тут, тому сторінки
 * панелі не рендеряться стороннім. Перевірка в layout, а не в proxy, бо
 * proxy не має доступу до бази.
 *
 * ⚠️ Цього НЕ досить для Server Actions: вони викликаються окремим POST і
 * layout при цьому не виконується. Кожна дія зобов'язана перевіряти сесію
 * сама — див. requireSession() у файлах actions.ts кожного розділу панелі.
 * Додаючи нову дію, починайте з цієї перевірки.
 */
